...
#define MAX_METADATA_LENGTH 40

char titleName[MAX_METADATA_LENGTH + 1] = "init";
char artistName[MAX_METADATA_LENGTH + 1] = "init";
...
static void bt_av_hdl_avrc_evt(uint16_t event, void *p_param)
{
	...
	{
		sprintf(titleName, "%.*s",MAX_METADATA_LENGTH, rc->meta_rsp.attr_text);
	}
	if (rc->meta_rsp.attr_id == ESP_AVRC_MD_ATTR_ARTIST)
	{
		sprintf(artistName, "%.*s",MAX_METADATA_LENGTH, rc->meta_rsp.attr_text);
	}
