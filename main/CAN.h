...
//private: no need to keep them here. They can be moved to the beginning of CAN.cpp
...
RX_CAN_BUFFER _CAN_RX_BUFFER; // This scary thing is a variable!
...
//extern CANClass CAN;
extern msgCAN CAN_TxMsg;
extern msgCAN CAN_RxMsg;
...
#define RESET(x)		gpio_set_level(x, 0)
//#define	SET(x)			_XS(x)
#define SET(x)			gpio_set_level(x, 1)


//#define	TOGGLE(x)		_XT(x) // ESP32 doesn't have an alternative for this!! Must kill!!

//#define	SET_OUTPUT(x)	_XSO(x) // This is implemented some other way
//#define	SET_INPUT(x)	_XSI(x) // This is implemented some other way
//#define	IS_SET(x)		_XR(x)
#define IS_SET(x)		gpio_get_level(x)
